...
规范化层最终应接收 Unicode 文本并保留原文到规范形式的可逆位置映射。阶段 1 只确定以下边界：大小写不改变发音规则；历史拼写 `I/J` 的辅音用法在发音层按 `j` 规则处理；附加符号和连字是否折叠、`U/V` 如何区分、组合字符采用何种正规形，均待独立规范化任务和测试确定。

在这些政策确定前，本文中的规则模式按规范化后的拉丁字母序列描述，不授权实现静默删除字符或改写词形。
...
penult 的轻重需要词汇数量或音节结构证据。`perseus-lewis-short` 可提供词汇证据，但每个词条必须记录具体定位；证据缺失时由后续诊断政策处理，本阶段不设默认猜测。
...
阶段 1 采用宽式 IPA 作为人类可读表示，并以同一符号集作为规范音素表的起点。`/e/`、`/o/` 是确定性占位的宽式类别，不声称排除实际朗读中的开闭变体。
...
| 鼻音 | `m n ɲ` |
| 流音 | `l r` |
| 组合 | `ks kʃ ksk`，以及按顺序保留的双辅音 |
| 韵律标记 | IPA 主重音 `ˈ`；音节边界 `.` 仅用于表示，不作为声学音素 |
...
| 重音位置 | `allen-greenough-accents`, Section 12 | 基线已记录 | 接入词汇数量并覆盖例外 |
| Unicode 与拼写规范化 | 尚待来源与独立任务 | 未实现 | 确定正规形、连字、附加符号和 `U/V` 政策 |
