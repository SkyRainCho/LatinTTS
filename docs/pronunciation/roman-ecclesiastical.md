...
`PronunciationPlan` 是带版本的消费契约。下游消费者消费前必须同时校验 `schema_version == "1"` 与 `rule_version == "ecclesiastical-roman-v1"`；不得静默消费不匹配的版本。`schema_version` 约束数据结构，`rule_version` 约束本规范的规则语义；任一值变化都要求调用方显式适配或拒绝处理。
...
两个 Liber 记录相同的 `authority_rank=1` 不构成可互相覆盖的平局：适用域先于数值排序。1962 发音规则节选决定规则语义；1961 全扫描只决定正文出现与印刷 acute 证据，不能用正文拼写反向改写发音规则。只有两个来源在同一适用域内提出冲突主张时，才需要继续按 locator、证据直接性和冲突决策记录处理。
...
`model_phonemes` 是逐 token 的模型输入序列：每个音节边界都保留 `.`，并在重读音节前插入独立 token `ˈ`。`ipa` 是供人阅读的连续字符串：在非首重读音节处，重读音节边界用 `ˈ` 取代 `.`，例如模型序列 `("d", "e", ".", "ˈ", "ʃ", "e", "n", ".", "d", "i", "t")` 对应 `deˈʃen.dit`。两者的音素顺序相同，但控制标记的渲染约定不同，维护者不得用简单的 token 拼接来判断二者不一致。
...
gold 集成测试是回归锚点和变更检测器：它证明当前管线仍与已批准标注一致，不是发音正确性的独立证明。`audit_gold_file()` 只验证 JSONL 结构、数量门禁、来源 ID、审核状态和其他可机械检查的不变量，也不判定 IPA 的语言学真伪。黄金记录中的 `syllables`、`stress_index` 和 `ipa` 是人工核验后的规范标注；可用工具辅助起草，但禁止从当前 `Pronouncer` 输出自动重写并把同一输出当作正确性证据。

任何规则、例外或音节算法变更必须先按 `rule_ids`、词形条件和转换路径列出受影响记录，人工对照受影响规则的规范来源及精确 locator，重新审核所有受影响词条，再更新 gold 与 `review_state`。若来源不足或审核尚未完成，记录必须保持候选或拒绝进入已批准 gold，不能通过批量重生成来消除测试差异。
